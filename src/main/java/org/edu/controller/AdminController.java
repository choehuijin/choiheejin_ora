...
public class AdminController {
   ...
   /**
    * 게시판생성 Insert(GET방식) 입니다.
    * 리퀘스트파람 = /admin/bodtype/update?bod_type <<
    * @throws Exception
    */
   ...
   /**
    * 게시판생성 Insert(POST방식) 입니다.
    * 
    * @throws Exception
    */
   @RequestMapping(value = "/bodtype/write", method = RequestMethod.POST)
 //overload 변수 이름 bodTypeInsert 위와 같은게 2개 존재
   //안에 들어있는 값이 다르기 때문에 가능
   ...
   public String bodTypeUpdate(@RequestParam String bod_type ,Locale locale, Model model) throws Exception {
      BoardTypeVO boardTypeVO = boardService.viewBoardType(bod_type);
   ...
   public String bodTypeList(Locale locale, Model model) throws Exception {
      ...
      /*[ List<boardTypeVO> list -> jsp boardTypeList; 이 데이터가 아래처럼 구성
          * {'notice','공지사항',1}
          * {'gallery','커뮤니티',2}
          * ]
          */
   ...
   public String boardList(@ModelAttribute("pageVO") PageVO pageVO, Locale locale, Model model, HttpServletRequest request) throws Exception {
      //초기 메뉴를 클릭시 /admin/board/list?searchBoard=notice
