...
<!-- 댓글에 사용되는 변수 함수 초기화 + 댓글 템플릿(빵틀) 출력 -->
       ...
       var printPageVO = function(pageVO, target){
          ...
          if(pageVO.prev)
         	 paging = paging + '<li class="page-item"><a class="page-link" href="'+(pageVO.start-1)+'">이전</a></li>';
       ...
       $(document).ready(function(){
          ...
          $(".pagination").on("click", "li a", function(event){
           ...
           <script>
      //댓글 입력,수정,삭제 버튼 이벤트처리 (Ajax를 이용해 URL호출-JSON데이터 get,set)
    $(document).ready(function(){
     ...
     <div id="modifyModal" class="modal modal-primary fade" role="dialog">
        <div class="modal-dialog">
         ...
         <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                   ...
                   <script>
                        //댓글 리스트에서 modify(댓글 수정버튼) 클릭 시 모달창 내용에 바인딩시키는 코딩
                        $(document).ready(function(){
                           //선택한 댓글(template:빵틀)의 데이터를 모달창의 id,클래스에 데이터 바인딩하는 코드(수정,삭제들어가기 전단계)
                           $(".timeline").on("click", ".replyLi", function(event){ //클릭하는데 .replyLi와 바인딩시킴.function값으로
                              var reply = $(this); 
